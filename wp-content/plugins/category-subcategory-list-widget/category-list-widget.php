...
class core_special_widgets_categories extends WP_Widget { 
    ...
    function widget($args, $instance) {
		wp_enqueue_style( 'categoryjquerycss', plugins_url( '/css/categoryjquerycss.css', __FILE__ ) );
		wp_enqueue_style( 'categorystyle', plugins_url( '/css/categorystyle.css', __FILE__ ) );
		wp_enqueue_script('jquery-ui-menu');
		wp_enqueue_script('categoryjs', plugins_url( '/js/categoryjs.js', __FILE__ ), array(), '1.0.0');
...
class Walker_Simple_Example2 extends Walker_Category {  
     ...
     function start_el(&$output, $item, $depth=0, $args=array(), $id = 0) { 
		...
		{
			if ($img == ''){
        		$output .= "<li class=\"list-group-item".$image_style."\"><a href='".esc_url( get_term_link( $item ) )."'><div style='float:left;margin-top:5px;' class='fontweightclasscustom fontsizeclasscustom'>".esc_attr( $item->name )."</div></a>";
			}
			else
			{
        		$output .= "<li class=\"list-group-item".$image_style."\"><a href='".esc_url( get_term_link( $item ) )."'><div style='float:left;' class='fontweightclasscustom fontsizeclasscustom'>".esc_attr( $item->name )."</div></a>";
			}
		}
    }  
...
function init()
{
	...
	wp_localize_script( 'categoryjs', 'ajax_object',
